<?php
// admin/akip.php - Kelola Dokumen AKIP
session_start();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'upload') {
    ...
    if (empty($judul)) {
        ...
        if (empty($error)) {
            $stmt = $pdo->query("SELECT IFNULL(MAX(urutan), 0) + 1 AS next_urutan FROM dokumen_akip WHERE tahun = '$tahun'");
            $row = $stmt->fetch();
            $next_urutan = $row['next_urutan'] ?? 1;
            
            $stmt = $pdo->prepare("INSERT INTO dokumen_akip 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['edit_id']) && $_POST['action'] == 'edit') {
    ...
    if (empty($judul)) {
        ...
        // Ambil data lama
        $stmt = $pdo->prepare("SELECT * FROM dokumen_akip WHERE id = ?");
        $stmt->execute([$id]);
        $old = $stmt->fetch();
        ...
        if (!$old) {
            ...
            if ($tipe_konten == 'file' && $file_uploaded) {
                ...
                if (!in_array($file_type, $allowed)) {
                    ...
                    if (!empty($old['file_dokumen']) && $old['file_dokumen'] != $new_file_name) {
                        $old_path = '../uploads/akip/' . $old['file_dokumen'];
            ...
            // === UPDATE DATABASE ===
            if (empty($error)) {
                $stmt = $pdo->prepare("UPDATE dokumen_akip 
...
if (isset($_GET['delete'])) {
    $id = (int)$_GET['delete'];
    $stmt = $pdo->prepare("SELECT file_dokumen, tipe_konten FROM dokumen_akip WHERE id = ?");
    $stmt->execute([$id]);
    $dok = $stmt->fetch();
    if ($dok) {
        if ($dok['tipe_konten'] == 'file' && !empty($dok['file_dokumen'])) {
            $file_path = '../uploads/akip/' . $dok['file_dokumen'];
            if (file_exists($file_path)) unlink($file_path);
        }
        $pdo->prepare("DELETE FROM dokumen_akip WHERE id = ?")->execute([$id]);
        $success = 'Dokumen berhasil dihapus!';
    }
    header('Location: akip.php?tahun=' . ($_GET['tahun'] ?? date('Y')));
    exit;
}
...
if (isset($_GET['toggle_status'])) {
    ...
    $stmt = $pdo->prepare("SELECT status FROM dokumen_akip WHERE id = ?");
    $stmt->execute([$id]);
    $dok = $stmt->fetch();
    if ($dok) {
        $new_status = $dok['status'] == 'aktif' ? 'nonaktif' : 'aktif';
        $pdo->prepare("UPDATE dokumen_akip SET status = ? WHERE id = ?")->execute([$new_status, $id]);
...
$stmt = $pdo->prepare("SELECT * FROM dokumen_akip WHERE tahun = ? ORDER BY urutan");
...
<head>
    ...
    <title>Kelola Akip - CEKIDOT</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .modal-box .edit-form .file-preview-wrapper .file-icon {
            font-size: 18px;
            color: #0f3b5e;
        }
        .modal-box .edit-form .file-preview-wrapper .file-name {
            flex: 1;
            font-size: 13px;
            color: #1e293b;
            word-break: break-all;
        }
        .modal-box .edit-form .file-preview-wrapper .file-size {
            font-size: 11px;
            color: #94a3b8;
        }
        ...
        .modal-box .edit-form .file-preview-wrapper .btn-remove-file:hover {
            background: #b91c1c;
        }
...
<main class="main-content">
    <div class="overlay">
        ...
        <div class="header">
            <div>
                <h1><i class="fas fa-clipboard-check"></i> Kelola Dokumen AKIP</h1>
...
function openViewModal(id, judul, deskripsi, tahun, tipe_konten, file_dokumen, link_url, status) {
    ...
    if (tipe === 'file' && file_dokumen) {
        var filePath = '../uploads/akip/' + file_dokumen;
        ...
        `;
        downloadBtn.href = '#';
        downloadBtn.removeAttribute('down<div class="filter-tahun">load');
