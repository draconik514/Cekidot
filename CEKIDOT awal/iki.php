<?php
// iki.php - Halaman IKI untuk Tamu/Guest
session_start();
...
// Ambil data dokumen IKI berdasarkan tahun (hanya yang aktif)
$stmt = $pdo->prepare("SELECT * FROM dokumen_iki WHERE tahun = ? AND status='aktif' ORDER BY urutan");
$stmt->execute([$tahun_aktif]);
$dokumen = $stmt->fetchAll();

// Total semua dokumen aktif
$stmt_total = $pdo->query("SELECT COUNT(*) as total FROM dokumen_iki WHERE status='aktif'");
$total_dokumen = $stmt_total->fetch()['total'];
?>

<!-- ============================================================
   OVERRIDE TITLE - Daftar Dokumen IKI - CEKIDOT
   ============================================================ -->
<script>
document.addEventListener('DOMContentLoaded', function() {
    document.title = 'Daftar Dokumen IKI - CEKIDOT';
});
</script>

<style>
    /* ============================================================
       HALAMAN IKI TAMU - MODERN & PROFESIONAL
       ============================================================ */
...
function openPreviewModal(judul, deskripsi, tipeKonten, status, fileName, linkUrl) {
    ...
    if (tipe === 'file' && fileName) {
        var filePath = 'uploads/iki/' + fileName;
