<?php
// akip.php - Halaman AKIP untuk Tamu/Guest
session_start();
...
// Ambil data dokumen AKIP berdasarkan tahun (hanya yang aktif)
$stmt = $pdo->prepare("SELECT * FROM dokumen_akip WHERE tahun = ? AND status='aktif' ORDER BY urutan");
$stmt->execute([$tahun_aktif]);
$dokumen = $stmt->fetchAll();

// Total semua dokumen aktif
$stmt_total = $pdo->query("SELECT COUNT(*) as total FROM dokumen_akip WHERE status='aktif'");
$total_dokumen = $stmt_total->fetch()['total'];
?>

<script>
document.addEventListener('DOMContentLoaded', function() {
    document.title = 'Daftar Dokumen AKIP - CEKIDOT';
});
</script>

<style>
    /* ============================================================
       HALAMAN AKIP TAMU - MODERN & PROFESIONAL
       ============================================================ */
    ...
    /* ============================================================
       MODAL PREVIEW - FULL SCREEN (SEDERHANA)
       ============================================================ */
    .modal-overlay {
        display: none;
        position: fixed;
    ...
    .modal-box .modal-header .left .title-group {
        flex: 1;
        min-width: 0;
    }
    ...
    .modal-box .modal-header .left .title-group .sub span {
        display: flex;
        align-items: center;
        gap: 4px;
    }
    ...
    .modal-box .modal-header .left .title-group .sub .badge.nonaktif {
        background: #fef2f2;
        color: #991b1b;
    }
    ...
    /* Info Sederhana - Hanya Judul & Deskripsi */
    ...
    .modal-box .modal-body .security-warning div strong {
        display: block;
    }
            ...
            <div class="content-wrapper">
                ...
                <div class="filter-tahun">
                    ...
                    <div class="tahun-items">
                        <?php foreach(range(2025, 2030) as $t): 
                            ...
                            $stmt_count = $pdo->prepare("SELECT COUNT(*) as total FROM dokumen_akip WHERE tahun = ? AND status='aktif'");
                ...
                <div class="dokumen-grid">
                    ...
                    <div class="dokumen-card" onclick="openPreviewModal(
                        ...
                        '<?= $d['tipe_konten'] ?? 'file' ?>',
                        '<?= $d['status'] ?? 'aktif' ?>',
                        '<?= addslashes(htmlspecialchars($file_name)) ?>',
                        ...
                        <div class="card-info">
                            ...
                            <div class="description"><?= htmlspecialchars($d['deskripsi']) ?></div>
                            <?php endif; ?>
                        </div>
...
<!-- ============================================================
   MODAL PREVIEW - TANPA TAHUN & NAMA FILE
   ============================================================ -->
...
function openPreviewModal(judul, deskripsi, tipeKonten, status, fileName, linkUrl) {
    ...
    if (tipe === 'file' && fileName) {
        var filePath = 'uploads/akip/' + fileName;
