...
<style>
    ...
    .monev-hero .container { position: relative; z-index: 1; }
    ...
    .monev-hero .hero-text { flex: 1; min-width: 280px; }
    ...
    .table-wrapper .table-header .table-badge i { margin-right: 4px; }
    ...
    .status-badge.efisien { background: #d1fae5; color: #065f46; border: 1px solid #6ee7b7; }
    .status-badge.tidak-efisien { background: #fef2f2; color: #991b1b; border: 1px solid #fca5a5; }
    ...
    @media (max-width: 992px) {
        .monev-hero .hero-content { flex-direction: column; align-items: flex-start; }
        .monev-content .content-wrapper { padding: 24px 20px; }
        .filter-wrapper .filter-group .btn-filter { padding: 4px 10px; font-size: 11px; }
    ...
    @media (max-width: 768px) {
        .monev-hero { padding: 30px 0 24px; }
        .monev-hero .hero-text h1 { font-size: 26px; }
        .monev-hero .hero-text p { font-size: 14px; }
        .monev-content .content-wrapper { padding: 16px 12px; }
        .filter-wrapper { padding: 8px 10px; gap: 6px; }
        .filter-wrapper .filter-group .btn-filter { padding: 3px 8px; font-size: 10px; }
    ...
    @media (max-width: 480px) {
        .monev-hero .hero-text h1 { font-size: 22px; }
        .filter-wrapper .filter-group .btn-filter { padding: 2px 6px; font-size: 9px; }
            ...
            <div class="content-wrapper">
                <div class="filter-wrapper">
                    <div class="filter-group">
                        ...
                        <a href="{{ route('monev.public', ['tahun' => $t, 'bulan' => $bulan_aktif, 'tab' => $tab_aktif]) }}" class="btn-filter {{ $tahun_aktif == $t ? 'active' : '' }}">
                            {{ $t }}
                        </a>
                        @endforeach
                    </div>
                    ...
                    <div class="filter-group">
                        @php $bulan_index = 0; @endphp
                        @foreach($bulan_list as $b)
                        <a href="{{ route('monev.public', ['tahun' => $tahun_aktif, 'bulan' => $b, 'tab' => $tab_aktif]) }}" class="btn-filter {{ $bulan_aktif == $b ? 'active' : '' }}">
                            {{ $bulan_singkat[$bulan_index] }}
                        </a>
                        @php $bulan_index++; @endphp
                        @endforeach
                    </div>
                    @endif
                </div>
                ...
                <div class="tab-nav">
                    <a href="{{ route('monev.public', ['tahun' => $tahun_aktif, 'bulan' => $bulan_aktif, 'tab' => 'bulanan']) }}" class="tab-btn {{ $tab_aktif == 'bulanan' ? 'active' : '' }}">
                        <i class="fas fa-list"></i> Data Bulanan
                    </a>
                    <a href="{{ route('monev.public', ['tahun' => $tahun_aktif, 'bulan' => $bulan_aktif, 'tab' => 'akumulasi']) }}" class="tab-btn {{ $tab_aktif == 'akumulasi' ? 'active' : '' }}">
                    ...
                    <div class="table-wrapper">
                        <div class="table-header">
                            ...
                            <span class="table-badge">
                                <i class="fas fa-database"></i> {{ $data_bulanan->count() }} data
                            </span>
                        </div>
                        <div class="table-scroll">
                            <table>
                                ...
                                <tbody>
                                    ...
                                    @else
                                    @php
                                        function getPredikatMonev($capaian) {
                                            if ($capaian === null || $capaian === '') {
                                                return ['label' => 'BELUM ADA', 'class' => 'predikat-belum'];
                                            }
                                            $capaian = (float) str_replace(',', '.', str_replace('.', '', $capaian));
                                            if ($capaian > 100) return ['label' => 'ISTIMEWA', 'class' => 'predikat-istimewa'];
                                            elseif ($capaian >= 80) return ['label' => 'BAIK', 'class' => 'predikat-baik'];
                                            elseif ($capaian >= 60) return ['label' => 'BUTUH PERBAIKAN', 'class' => 'predikat-butuh'];
                                            elseif ($capaian >= 20) return ['label' => 'KURANG', 'class' => 'predikat-kurang'];
                                            elseif ($capaian > 0) return ['label' => 'SANGAT KURANG', 'class' => 'predikat-sangat'];
                                            else return ['label' => 'BELUM ADA', 'class' => 'predikat-belum'];
                                        }
                                    @endphp
                                    @php $no = 1; @endphp
                                    @foreach($data_bulanan as $row)
                                    @php
                                        $pred_ik = getPredikatMonev($row->capaian_ik);
                                        $pred_keu = getPredikatMonev($row->capaian_keu);
                                    @endphp
                                    <tr>
                                        <td class="text-center">{{ $no++ }}</td>
                                        <td class="text-left">{{ $row->sub_kegiatan }}</td>
                                        ...
                                        <td>
                                            @if(!empty($row->sumber_data))
                                                <a href="{{ $row->sumber_data }}" target="_blank" class="sumber-link">
                                                    <i class="fas fa-external-link-alt"></i> {{ substr($row->sumber_data, 0, 35) }}{{ strlen($row->sumber_data) > 35 ? '...' : '' }}
                                                </a>
                                            @else
                                                <span style="color:#94a3b8; font-size:10px;">-</span>
                                            @endif
                                        </td>
                                        <td class="text-left">{{ $row->faktor_penghambat ?? '-' }}</td>
                                        <td class="text-left">{{ $row->faktor_pendukung ?? '-' }}</td>
                                    </tr>
                                    @endforeach
                                    @endif
                                </tbody>
                        ...
                        <div class="table-scroll">
                            <table>
                                ...
                                <tbody>
                                    ...
                                    @else
                                    @php
                                        function getPredikatAkumulasi($capaian) {
                                            if ($capaian === null || $capaian === '') {
                                                return ['label' => 'BELUM ADA', 'class' => 'predikat-belum'];
                                            }
                                            $capaian = (float) str_replace(',', '.', str_replace('.', '', $capaian));
                                    ...
                                    @php
                                        $pred_ik = getPredikatAkumulasi($row->capaian_ik);
                                        $pred_keu = getPredikatAkumulasi($row->capaian_keu);
                                    @endphp
                                    <tr>
                                        <td class="text-center">{{ $no++ }}</td>
                                        <td class="text-left">{{ $row->sub_kegiatan }}</td>
