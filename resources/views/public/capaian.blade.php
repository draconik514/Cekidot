...
<style>
    ...
    .capaian-hero .container { position: relative; z-index: 1; }
    ...
    .capaian-hero .hero-text { flex: 1; min-width: 280px; }
    ...
    .capaian-hero .hero-stats .stat { text-align: center; padding: 0 6px; }
    ...
    .capaian-hero .hero-stats .stat .number.high { color: #4ade80; }
    .capaian-hero .hero-stats .stat .number.low { color: #f87171; }
    ...
    .sumber-cell i { color: #0f3b5e; margin-right: 4px; }
    .sumber-cell a {
        color: #0f3b5e;
        text-decoration: none;
    }
    ...
    @media (max-width: 992px) {
        .capaian-hero .hero-content { flex-direction: column; align-items: flex-start; }
        .capaian-hero .hero-stats { width: 100%; justify-content: center; }
        .capaian-content .content-wrapper { padding: 24px 20px; }
    ...
    @media (max-width: 768px) {
        .capaian-hero { padding: 30px 0 24px; }
        .capaian-hero .hero-text h1 { font-size: 26px; }
        .capaian-hero .hero-text p { font-size: 14px; }
        .capaian-hero .hero-stats { padding: 10px 16px; gap: 16px; }
        .capaian-hero .hero-stats .stat .number { font-size: 20px; }
        .capaian-hero .hero-stats .stat .label { font-size: 9px; }
        .capaian-hero .hero-stats .stat-divider { display: none; }
        .capaian-content .content-wrapper { padding: 16px 12px; }
        ...
        .legend-wrapper { flex-direction: column; align-items: flex-start; gap: 4px; }
    }
    @media (max-width: 480px) {
        .capaian-hero .hero-text h1 { font-size: 22px; }
        .capaian-hero .hero-stats { flex-wrap: wrap; gap: 12px; justify-content: center; }
        .capaian-hero .hero-stats .stat { flex: 1; min-width: 60px; }
        .capaian-hero .hero-stats .stat .number { font-size: 18px; }
        ...
        <div class="container">
            <div class="content-wrapper">
                <div class="filter-tahun">
                    <button class="btn-tahun" onclick="window.location.href='{{ route('capaian.public') }}?tahun={{ $tahun_aktif - 1 }}'" {{ $tahun_aktif <= 2025 ? 'disabled' : '' }}>
                        <i class="fas fa-chevron-left"></i>
                    </button>
                    
                    <div class="tahun-items">
                        @foreach($tahun_list as $t)
                        <a href="{{ route('capaian.public') }}?tahun={{ $t }}" class="tahun-item {{ $t == $tahun_aktif ? 'active' : '' }}">
                            {{ $t }}
                        </a>
                        @endforeach
                    </div>
                    ...
                    <button class="btn-tahun" onclick="window.location.href='{{ route('capaian.public') }}?tahun={{ $tahun_aktif + 1 }}'" {{ $tahun_aktif >= 2030 ? 'disabled' : '' }}>
                ...
                <div class="table-wrapper">
                    <table>
                        ...
                        <tbody>
                            ...
                            @else
                            @php
                                function getPredikatCapaian($capaian) {
                                    if ($capaian > 100) {
                            ...
                            @php
                                ...
                                $predikat = getPredikatCapaian($capaian);
                                $target_display = formatAngkaFlexibel($d->target);
                                $realisasi_display = formatAngkaFlexibel($d->realisasi);
                            @endphp
                            <tr>
                                <td class="program-cell">{{ $d->program }}</td>
                                <td class="sasaran-cell">{{ $d->sasaran }}</td>
                                <td class="indikator-cell">{{ $d->indikator }}</td>
                                ...
                                <td class="text-center">
                                    <span class="frekwensi-badge">{{ $d->frekwensi }}</span>
                                </td>
                                <td class="sumber-cell">
                                    @if(!empty($d->sumber_data))
                                        @if(filter_var($d->sumber_data, FILTER_VALIDATE_URL))
                                            <a href="{{ $d->sumber_data }}" target="_blank" title="{{ $d->sumber_data }}">
                                                <i class="fas fa-external-link-alt"></i> Link
                                            </a>
                                        @else
                                            <i class="fas fa-info-circle"></i> {{ substr($d->sumber_data, 0, 30) }}{{ strlen($d->sumber_data) > 30 ? '...' : '' }}
                                        @endif
                                    @else
                                ...
                                <td class="pj-cell">{{ $d->penanggung_jawab }}</td>
                            </tr>
                            @endforeach
                            @endif
                        </tbody>
