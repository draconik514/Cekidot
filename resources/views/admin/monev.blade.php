...
<style>
    ...
    .akumulasi-section .akumulasi-header .akumulasi-auto i { margin-right: 4px; }
    ...
    .akumulasi-section table .capaian-value {
        font-weight: 700;
    }
    .akumulasi-section table .capaian-value.istimewa { color: #1d4ed8; }
    .akumulasi-section table .capaian-value.baik { color: #16a34a; }
    .akumulasi-section table .capaian-value.butuh-perbaikan { color: #92400e; }
    .akumulasi-section table .capaian-value.kurang { color: #9a3412; }
    .akumulasi-section table .capaian-value.sangat-kurang { color: #dc2626; }
    .akumulasi-section table .capaian-value.belum-ada { color: #94a3b8; }
...
<div class="filter-wrapper">
    <div class="filter-group">
        ...
        <a href="{{ route('admin.monev.index', ['tahun' => $t, 'bulan' => $bulan_aktif, 'tab' => $tab_aktif]) }}" class="btn-filter {{ $tahun_aktif == $t ? 'active' : '' }}">
            {{ $t }}
        </a>
        @endforeach
    </div>
    ...
    <div class="filter-group">
        ...
        <a href="{{ route('admin.monev.index', ['tahun' => $tahun_aktif, 'bulan' => $b, 'tab' => $tab_aktif]) }}" class="btn-filter {{ $bulan_aktif == $b ? 'active' : '' }}">
...
<!-- Tab Navigation -->
<div class="tab-nav">
    <a href="{{ route('admin.monev.index', ['tahun' => $tahun_aktif, 'bulan' => $bulan_aktif, 'tab' => 'bulanan']) }}" class="tab-btn {{ $tab_aktif == 'bulanan' ? 'active' : '' }}">
        <i class="fas fa-list"></i> Input Data
    </a>
    <a href="{{ route('admin.monev.index', ['tahun' => $tahun_aktif, 'bulan' => $bulan_aktif, 'tab' => 'akumulasi']) }}" class="tab-btn {{ $tab_aktif == 'akumulasi' ? 'active' : '' }}">
...
<div class="tab-content {{ $tab_aktif == 'bulanan' ? 'active' : '' }}">
    <form method="post" enctype="multipart/form-data" autocomplete="off" id="mainForm" action="{{ route('admin.monev.update') }}">
        @csrf
            ...
            <div class="table-scroll">
                <table>
                    ...
                    <tbody id="tableBody">
                        ...
                        @php
                            $pred_ik = App\Http\Controllers\Admin\MonevController::getPredikatStatic($row->capaian_ik);
                            $pred_keu = App\Http\Controllers\Admin\MonevController::getPredikatStatic($row->capaian_keu);
...
document.getElementById('confirmDeleteBtn').addEventListener('click', function() {
    if (deleteId !== null) {
        ...
        form.method = 'POST';
        form.action = '{{ route('admin.monev.delete') }}';
        form.innerHTML = '<input type="hidden" name="delete_id" value="' + deleteId + '">' +
            '<input type="hidden" name="tahun" value="{{ $tahun_aktif }}">' +
            '<input type="hidden" name="bulan" value="{{ $bulan_aktif }}">' +
            '<input type="hidden" name="tab" value="{{ $tab_aktif }}">@csrf';
