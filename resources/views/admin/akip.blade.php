...
@section('title', 'Kelola AKIP - CEKIDOT')

@section('styles')
<style>
    /* ===== STYLE SAMA DENGAN AKIP.PHP ASLI ===== */
    .header {
        display: flex;
...
<div class="filter-tahun">
    ...
    <div class="tahun-items" id="tahunItems">
        @foreach(range(2025, 2030) as $t)
            ...
            @endphp
            <a href="{{ route('admin.akip.index', ['tahun' => $t]) }}" class="tahun-item {{ $is_active ? 'active' : '' }}" data-year="{{ $t }}">
            ...
            <tr>
                ...
                <td>
                    <div class="action-group">
                        ...
                        <a href="{{ route('admin.akip.toggle', $d->id) }}?tahun={{ $tahun_aktif }}" class="btn-action btn-toggle {{ $d->status ?? 'aktif' }}" title="{{ ($d->status ?? 'aktif') == 'aktif' ? 'Nonaktifkan' : 'Aktifkan' }}">
...
<div class="modal-overlay" id="editModal">
    <div class="modal-box">
        ...
        <form method="post" enctype="multipart/form-data" class="edit-form" id="editForm" action="{{ route('admin.akip.update') }}">
            @csrf
...
function openViewModal(id, judul, deskripsi, tahun, tipe_konten, file_dokumen, link_url, status) {
    ...
    if (tipe === 'file' && file_dokumen) {
        var filePath = '{{ asset('storage/uploads/akip') }}/' + file_dokumen;
...
function openDeleteModal(id, judul) {
    ...
    document.getElementById('deleteConfirmBtn').href = '{{ url('admin/akip/delete') }}/' + id + '?tahun={{ $tahun_aktif }}';
...
function changeYear(year) {
    ...
    setTimeout(function() {
        window.location.href = '{{ route('admin.akip.index') }}?tahun=' + year;
    }, 250);
}
